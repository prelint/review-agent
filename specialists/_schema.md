...
| `fingerprint` | yes | Exactly `path:anchor:category`. Join the `path`, `anchor` and `category` fields on this same object with colons. Copy them, do not retype them. Stage 2 re-derives the canonical value from the three fields, so a mismatch is corrected and named, not fatal. A missing `path`, `anchor` or `category` drops the finding. See below. |
